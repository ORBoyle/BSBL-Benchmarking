...
def ravel_list(d):
    ...
    for i in range(d.shape[0]): #xrange
        r = np.r_[r,d[i]]
    return r
...
class bo:
    ...
    def fit_transform(self, X, y, blk_start_loc=None):
        ...
        if blk_start_loc.any()==None: #added the .any()
            blkLen = int(N/16.)
        ...
        block_slice = np.array([blk_start_loc[i] + np.arange(blk_len_list[i]) for i in range(nblock)]) #xrange
        ...
        # loops
        for count in range(self.max_iters): #xrange
